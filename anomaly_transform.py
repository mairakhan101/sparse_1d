"Implementation from https://github.com/spencerbraun/anomaly_transformer_pytorch"
...
class AnomalyAttention(nn.Module):
    def __init__(self, N, d_model):
        ...
        self.Wq = nn.Linear(d_model, d_model, bias=False).cuda()
        self.Wk = nn.Linear(d_model, d_model, bias=False).cuda()
        self.Wv = nn.Linear(d_model, d_model, bias=False).cuda()
        self.Ws = nn.Linear(d_model, 1, bias=False).cuda()

        self.Q = self.K = self.V = self.sigma = torch.zeros((N, d_model)).cuda()

        self.P = torch.zeros((N, N)).cuda()
        self.S = torch.zeros((N, N)).cuda()
...
class AnomalyTransformer(nn.Module):
    def __init__(self, N, d_model, layers, lambda_):
        ...
        self.blocks = nn.ModuleList(
            [AnomalyTransformerBlock(self.N, self.d_model).to('cuda') for _ in range(layers)]
        )
        self.output = None
    ...
    def anomaly_score(self, x):
        x = x.to('cuda')
        self.output = self.output = self.output.to('cuda')
